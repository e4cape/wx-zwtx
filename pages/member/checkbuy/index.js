// pages/member/checkbuy/index.js
var t = getApp(),
  e = t.requirejs("core");
...
Page({
  ...
  onLoad: function (options) {
    ...
    //console.log(options.cartStoreId)
    var that = this;
    wx.request({
      //请求服务器注册接口
      url: t.globalData.api + 'mall/cart/queryCart?token=' + token,
      //定义传到后台的数据
      data: {
          ...
          wx.showToast({
            ...
            success: function () {
              setTimeout(function () {
                //要延时执行的代码
                wx.navigateTo({
                  url: '/pages/member/address/index'　　//跳转到注册页面
                })
              }, 3000) //延迟时间
            }
          })
  ...
  addressList: function (e) {
    ...
    wx.request({
      ...
      success: function (res) {
        // console.log(res.data.data[0].addressList);
        var addressList = res.data.data[0].addressList;
        for (var i in addressList) {
  ...
  checkbuy: function (e) {
    ...
    var list = this.data.goodsList[0].list;
    ...
    for (var i in list) {
      cartList[i] = 
        {
          cartBuyerId: memberId,
          cartGoodsId: list[i].goodsId,
          cartGoodsNorms: list[i].cartGoodsNorms,
          cartGoodsSpec: list[i].cartGoodsSpec,
          cartGoodsSpecId: list[i].cartGoodsSpecId,
          cartId: list[i].cartId,
          cartNum: list[i].cartNum,
          cartState: list[i].cartState,
          cartStoreId: storeId,
          goodsCateId: list[i].goodsCateId,
          goodsImage: list[i].goodsDefaultImage,
